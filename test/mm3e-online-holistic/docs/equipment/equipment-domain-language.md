...
- A device is a permanent part of the character's trait set — the character pays power points for it; if lost or destroyed the character can replace it because it is considered a permanent part of their concept.
...
- The GM may require a hero point to temporarily use a device belonging to another character, analogous to performing a power stunt without suffering fatigue.
- Characters with the Inventor or Artificer advantage can create temporary devices for single-adventure use.
...
- Equipment is limited to technology commonly available in the setting; the GM decides what qualifies; advanced items (battlesuits, anti-gravity devices) are devices, not equipment.
- Equipment bonuses do not stack with each other or with other bonus types; only the highest bonus applies.
- Equipment cannot benefit from extra effort; a hero point must be spent instead to perform an equipment "power stunt."
- Equipment is subject to damage, malfunction, and loss with impunity; the GM may declare it fails or is taken as a complication.
- Characters pay equipment points only for adventuring gear with a direct impact on their heroic role; everyday possessions are background color.
- Most equipment provides the Features effect or other minor effects at 1 point per Feature.
...
- The Equipment advantage sets the pool limit; all items are acquired within this pool.
...
- If a Removable device is permanently lost, the GM should allow the character to reallocate those power points.
...
- Losing the battlesuit means losing all powers tied to it.
...
- In more realistic settings the GM may charge for Immunity (wearer's powers) as a 1-point Feature.
...
- A completed invention lasts one scene; reuse costs a hero point (no further checks) or the character spends power points to make it permanent.
...
- Each weapon has traits: category, effect, critical threat range, and equipment point cost.
...
- Alternate Vehicles: pay full cost for the most expensive, 1 point per additional vehicle of equal or lesser cost; a hero point can produce an additional vehicle on hand.
- Standard amenities (seating, seatbelts, radio, headlights, climate control) are included at no cost.
...
- Vehicle power effects cost one-fifth normal power cost because they are paid in equipment points.
...
- Self-repairing taken twice allows the HQ to rebuild itself within a week if destroyed.
...
- Constructs lack either (a) Intellect and Presence — automatons, immune to Will and interaction effects, fail INT/PRE checks — or (b) Strength and Agility — immobile intellects that cannot perform physical actions.
...
- Constructs larger or smaller than Medium pay power points for Innate and Permanent Growth or Shrinking.
