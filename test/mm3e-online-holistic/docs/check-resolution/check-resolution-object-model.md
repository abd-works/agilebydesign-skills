...
+ Trait(rank: Rank, difficultyLadder: DifficultyLadder, character: Character)
------
+ rank: Rank
+ << composition >> difficultyLadder: DifficultyLadder
+ character: Character
----
+ makeCheck(dc: DifficultyClass, circumstance: CircumstanceModifier): CheckResult
	Invariant: modifier = rank.value
...
+ toMeasure(type: MeasurementType): Number
	Interaction:
		entry: RankedMeasurement = Measurement.lookup(rank: this, type: type)
		return entry.value
+ add(other: Rank, type: MeasurementType): Rank
	Invariant: ranks must not be added directly — convert to measures first
	Interaction:
		...
		result: Rank = Measurement.rankFor(measure: combined, type: type)
		return result
...
+ RankedMeasurement(type: MeasurementType, rank: Rank, value: Number, units: String)
------
...
+ << composition >> rankedMeasurements: List<RankedMeasurement>
	Invariant: one real-world value for every rank for this measurement type
----
+ lookup(rank: Rank, type: MeasurementType): RankedMeasurement
...
+ rankFor(measure: Number, type: MeasurementType): Rank
	Invariant: returns the Rank whose measurement value is closest to the given measure for the given type
+ calculate(operatorRank: Rank, scaleRank: Rank, type: MeasurementType): Number
	Invariant: look up value for operator rank and scale rank from the type's table
	Interaction:
...
+ Check(trait: Trait, dc: DifficultyClass, circumstanceModifier: CircumstanceModifier)
------
+ trait: Trait
+ dc: DifficultyClass
...
+ CheckResult(roll: Integer, rollTotal: Integer, isSuccess: Boolean, check: Check)
	Interaction:
		margin = rollTotal − check.dc.value
------
+ roll: Integer
+ rollTotal: Integer
+ isSuccess: Boolean
...
----
+ supplyDcToCheck(stage: DifficultyStage): DifficultyClass

-----

#### **DifficultyStage**  << ValueObject >>

+ DifficultyStage(descriptor: String, dcValue: DifficultyClass, taskDescription: String)
------
+ descriptor: String
+ dcValue: DifficultyClass
+ taskDescription: String
----
+ supplyDc(): DifficultyClass
...
+ resolve(): CheckResult
	...
	Interaction:
		...
		dc = new DifficultyClass(value: opposingResult.rollTotal)
...
+ TeamCheck(trait: Trait, dc: DifficultyClass, helpers: List<Check>)
------
+ << aggregation >> helpers: List<Check>
----
+ resolveHelpers(): CircumstanceModifier
	...
	Interaction:
		helperDC: DifficultyClass = new DifficultyClass(value: 10)
		successCount: Integer = helpers.count(h: new Check(trait: h.trait, dc: helperDC).resolve().isSuccess)
...
------
+ character: Character
+ << aggregation >> appliedConditions: List<ImposedCondition>
----
...
+ PowerEffect(rank: Rank, resistanceTrait: Trait)
------
