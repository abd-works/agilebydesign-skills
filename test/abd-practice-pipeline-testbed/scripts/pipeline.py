...
TESTBED_ROOT = SCRIPT_DIR.parent
REPO_ROOT = TESTBED_ROOT.parent.parent  # agilebydesign-skills/
...
def step_render(graph: Path, drawio: Path, mode: str = "outline") -> int:
    """Step 5: render the drawio diagram from JSON."""
    print(f"[step 5] render {drawio} (mode={mode})")
...
def cmd_validate(args: argparse.Namespace) -> int:
    cfg = load_workspace_config(TESTBED_ROOT)
...
def cmd_on_edit(args: argparse.Namespace) -> int:
    ...
    if path.name.endswith(cfg.graph_suffix):
        # graph changed independently -> render
        cascade = cfg.json_to_drawio
        if gate_decision("json_to_drawio", cascade, non_interactive=True):
            return step_render(path, drawio_path_for(path, cfg))
        return 0
