...
class StoryMap:
    ...
    def from_json_file(cls, path: Path | str) -> 'StoryMap':
        """Load from a ``story-graph.json`` (or equivalent) file on disk."""
        import json
        p = Path(path)
        if not p.is_file():
    ...
    def from_bot(cls, bot: Any) -> 'StoryMap':
        """Load using agile_bots path layout when ``story_graph.story_graph_paths`` is importable."""
        import json
        ...
        if hasattr(bot, 'bot_paths') and hasattr(bot.bot_paths, 'story_graph_paths'):
            ...
            wd = bot.bot_paths.workspace_directory
            ...
            try:
                ...
                paths = StoryGraphPaths(wd, bot_name)
                story_graph_path = paths.story_graph_path
            except ImportError:
                story_graph_path = Path(wd) / bot_name / 'docs' / 'story' / 'story-graph.json'
