...
|-----------|----------------|
| Epic: Create Abd-Skill | AbdSkill, AssembledAgent, BuildScript, agile-skill-build |
| Epic: Initialize Agile Context Engine | AgileContextEngine, AbdSkill, RuleSet, Workspace |
| Epic: Add Context to Memory | ContentSource, Markdown, Chunk, Memory, Memories, Workspace |
| Epic: Use Synthesizer Skill | ContextSources, Strategy, Slice, InteractionTree, StateModel |

### Path (Cross-cutting)

**Applies to:** All epics (file paths used throughout). *See `docs/abd-architecture-pattern.md` §1.1.5 for full Path convention.*
...
### AgileContextEngine, AbdSkill, RuleSet, Workspace, ArchitectureConstraint

**Applies to:** Epic: Initialize Agile Context Engine (all stories); Epic: Create Abd-Skill (AbdSkill shared).
...
AbdSkill
- Engine engine
     Injected at construction. AbdSkill uses it for context (workspace, strategy_path, slice_index, etc.).
- Path path
     Path to skill directory
     invariant: skills/abd-<skill_name>/
- RuleSet rule_set
     RuleSet (markdown + JSON)
     invariant: skills/abd-<skill_name>/rules
- Script[] scripts
     Scripts collection
     invariant: skills/abd-<skill_name>/scripts/
- String core, process, strategy, output, validation
     Content from content/*.md
     invariant: from skills/abd-<skill_name>/content/
...
- Path[] markdown_paths
     ...
     invariant: Path objects for core.md, process.md, strategy.md, output.md, validation.md
     Specific markdown file eg
     - Object scanner_rules
          JSON for scanners
          invariant: from skills/abd-<skill_name>/rules
...
**Applies to:** Epic: Create Abd-Skill — Story: Create scaffolding via script, Story: AI reruns build script.

*AbdSkill is defined above (Initialize). Create Skill adds:*

```
AssembledAgent
- Path path
     Path to assembled output
     invariant: skills/abd-<skill_name>/AGENTS.md (or assembled agent file under skill); returns Path, not String
- String content
     Merged content for agent consumption
     invariant: core + process + strategy + output + validation assembled in order

BuildScript
- Path path
     Path to build script
     invariant: skills/abd-<skill_name>/scripts/build.py; returns Path, not String
...
### Use Synthesizer Skill — ContextSources, Strategy, Slice

**Applies to:** Epic: Use Synthesizer Skill (all stories: Create Synthesizer Strategy, Generate Slices, Improve Strategy, Improve Skill). Synthesizer gets context from Memories — not from raw paths.
...
- RuleSet rule_set
     From abd-skill (synthesizer instructions)
- ContextSources gather(content_sources, workspace, strategy): ContextSources
     ContentSources, Workspace, Strategy
     Populates memory (convert, chunk, sync via abd-context-to-memory); refers to memories for chunks; assembles with workspace, strategy, rule_set per synthesizer phase (strategy vs slice)
...
**Instruction injection (what to inject, when):** See `skills/abd-story-synthesizer/docs/abd-instruction-injection-analysis.md`. Summary:
...
| Note | Applies to |
|------|------------|
| Object-oriented model, Abd-skill, Rule dual representation, Create Skill | Epic: Create Abd-Skill; Epic: Initialize Agile Context Engine |
| Python/JSON hybrid, Architecture-pattern constraints | Epic: Initialize Agile Context Engine |
| Workspace, Skill outputs | All epics (Initialize sets workspace; Add Context and Use Synthesizer run in workspace) |
| Context from memories, not raw paths | Epic: Add Context to Memory; Epic: Use Synthesizer Skill |
...
- **Path concept:** All file-returning properties return Path objects (not strings). Path handles OS separators and centralizes path logic. When integrating with agile_bots, reuse `agile_bots/src/bot_path` (BotPath, StoryGraphPaths).
