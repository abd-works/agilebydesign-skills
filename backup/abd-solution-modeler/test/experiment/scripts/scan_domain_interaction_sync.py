...
Rule file: test/experiment/rules/domain-interaction-sync.md
...
Usage (standalone):
    python test/experiment/scripts/scan_domain_interaction_sync.py [--input <path>]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=f"Domain-interaction sync scanner. Rule: {RULE_ID}"
    )
    parser.add_argument(
        "--input",
        default=str(experiment_dir / "step2-output.json"),
    )
    ...
    print(f"  Rule: test/experiment/rules/domain-interaction-sync.md\n")
    for v in violations:
