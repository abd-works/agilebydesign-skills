<#
.SYNOPSIS
  Create directory junctions so Cursor discovers the module-partitioning skill globally
  (~/.cursor/skills/) and under agilebydesign-skills/.cursor/skills/.

.DESCRIPTION
  Canonical source: agents/abd-ooad/skills/module-partitioning/. Junctions point here — no copies.
...
# module-partitioning -> skills -> abd-ooad -> agents -> repo root
...
if (-not $ProjectOnly) {
    ...
    New-JunctionSafe -Link "$userSkills\module-partitioning" -Target $skillRoot
}

if (-not $UserOnly) {
    Write-Host "Project-level junction (.cursor/skills/):" -ForegroundColor Cyan
    New-JunctionSafe -Link "$projectSkills\module-partitioning" -Target $skillRoot
}
