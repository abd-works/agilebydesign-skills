<#
.SYNOPSIS
  Create directory junctions so Cursor discovers the domain-model-skeleton skill globally
  (~/.cursor/skills/) and under agilebydesign-skills/.cursor/skills/.

.DESCRIPTION
  Canonical source: agents/abd-ooad/skills/domain-model-skeleton/. Junctions point here — no copies.
...
# domain-model-skeleton -> skills -> abd-ooad -> agents -> repo root
...
if (-not $ProjectOnly) {
    ...
    New-JunctionSafe -Link "$userSkills\domain-model-skeleton" -Target $skillRoot
}

if (-not $UserOnly) {
    Write-Host "Project-level junction (.cursor/skills/):" -ForegroundColor Cyan
    New-JunctionSafe -Link "$projectSkills\domain-model-skeleton" -Target $skillRoot
}
