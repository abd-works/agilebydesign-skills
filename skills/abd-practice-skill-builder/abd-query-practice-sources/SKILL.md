...
5. **Write `inputs/abd-answers-retrieval.md`** — required. Ensure the target **`skills/<skill-name>/inputs/`** folder exists. Start from **`agents/abd-practice-skill-builder/skills/abd-query-practice-sources/templates/abd-answers-retrieval-input.md`** (copy into the target **`inputs/`** if the log file is new). For **each kept chunk**, add **`### Kept chunk N`** with the metadata fields in **Output log** above and paste the **verbatim** body in a fenced block. Merge all queries into **one** file; **dedupe** identical **Source location** / slide bodies.
