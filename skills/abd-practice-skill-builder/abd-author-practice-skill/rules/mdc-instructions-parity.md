...
- Run **`mdc-instructions-parity-scanner`** on the **target** skill root when validating ( **`--skill-root`** = **abd-author-practice-skill**, **`--workspace`** = **`skills/<target>/`** ).
  ...
  **Example (pass):** `python skills/execute-skill-using-skills-rules/scripts/run_scanners.py --skill-root agents/abd-practice-skill-builder/skills/abd-author-practice-skill --workspace C:/dev/agilebydesign-skills/skills/correct_output` reports **PASS** for **mdc-instructions-parity**.
