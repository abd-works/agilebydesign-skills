...
| 3b  | Story Definition → Acceptance Tests → Engineering | Same slice | Per-story | Drain the slice |
"""
...
def test_parse_plan_normalizes_stage_aliases():
    runs = parse_plan(_SAMPLE_PLAN)
    # "Story Definition" -> scenarios; "Acceptance Tests" -> acceptance-tests
    assert runs[3].stages == ["scenarios", "acceptance-tests", "engineering"]
