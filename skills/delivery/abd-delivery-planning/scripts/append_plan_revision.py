#!/usr/bin/env python3
"""Append an entry to `<workspace>/agile-delivery-plan.changelog.md`.
...
_AGENT_ROOT = _SKILL_ROOT.parent.parent  # skills/<name>/scripts -> three up
_CONFIG = _AGENT_ROOT / "skill-config.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--changelog", type=Path, default=None,
                    help="Explicit path to the changelog file (defaults to <workspace>/agile-delivery-plan.changelog.md).")
    ...
    changelog_path = ns.changelog.resolve() if ns.changelog else workspace / "agile-delivery-plan.changelog.md"
