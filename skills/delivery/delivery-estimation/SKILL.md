...
Before estimating, the team must agree on **which delivery stages the estimate covers**. The stages below are the standard set; defaults apply unless the team opts in or out at session start.

- **Story mapping** — placing the item in the backlog hierarchy. *Not included by default.*
- **Acceptance criteria definition** — writing WHEN/THEN AC for the story. *Included by default.*
- **Specification definition** — writing specification-by-example scenarios. *Included by default.*
- **Development** — implementing the production code. *Included by default.*
- **Story testing** — acceptance tests / system integration tests for the story. *Included by default.*
- **Regression testing** — running and maintaining the broader regression suite. *Included by default.*
- **User testing** — end-user or UAT validation. *Not included by default.*
...
Default guidelines when estimating stories (coverage = development + acceptance testing):

- **Points schemes:** > 5 points → suggest splitting into smaller stories
- **Day schemes:** > 8 days → suggest splitting into smaller stories

"Days" here means the full story lifecycle: development through system integration test and acceptance test — not just coding time. The threshold is a conversation trigger, not a hard rule: the team may keep a story whole if the estimate is confident and the contributing factors are well-understood. But crossing the threshold should always prompt the question: *Can this be broken into independently deliverable pieces?*

When a story crosses the threshold, use **`abd-thin-slicing`** to break it into thin slices. Each slice is estimated separately. The original story is marked "split — see children" in the session file.
...
- **New acceptance criteria** — discussion reveals missing WHEN/THEN behaviour on an existing item. Record the AC and feed it to **`abd-acceptance-criteria`**.
- **New items** — the team discovers work that was not on the map at all (a missing story, a new epic). Record it and feed it to **`abd-story-mapping`** to place it in the hierarchy.
...
Scope              — granularity (epic / sub-epic / story / thin slice)
                     ...
                     coverage boundary checklist:
                       [ ] story mapping (not default)
                       [x] AC definition
                       [x] spec definition
                       [x] development
                       [x] story testing
                       [x] regression testing
                       [ ] user testing (not default)
...
1. **Establish the session scope.** Agree with the team on what is being estimated and at what granularity (epics, sub-epics, stories, thin slices). Review the delivery stages and confirm which are included — the defaults are acceptance criteria definition, specification definition, development, story testing, and regression testing. Opt in to story mapping or user testing if the team wants those covered. Record the scope and coverage in the session setup using **`templates/estimation-session.md`**.
...
```powershell
.\agents\abd-practice-skill-builder\skills\abd-author-practice-skill\scripts\Deploy-SkillOutputs.ps1 -SkillPath skills/delivery/delivery-estimation -ProjectRoot <target-project> -Force
```
...
python skills/execute-skill-using-skills-rules/scripts/run_scanners.py \
  ...
  --workspace /absolute/path/to/repo/skills/delivery/delivery-estimation
```
