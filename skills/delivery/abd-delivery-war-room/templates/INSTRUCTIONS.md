...
Read `delivery-war-room/slot-NN-start.md` for `team-role`, scope, stage, skills, corrections, entry conditions, and early question triggers.
...
After confirmation, update `story-graph.json` via `story-graph-ops` for stages that produce graph content (discovery, prioritization, exploration, scenarios, acceptance-tests).

## 7) When done

Write `delivery-war-room/slot-NN-finished.md` with:
- Timestamp
- All artifact paths produced
