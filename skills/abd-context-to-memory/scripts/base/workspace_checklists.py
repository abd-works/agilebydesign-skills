"""
Ensure live workflow checklists under <CONTENT_MEMORY_ROOT>/<skill_name>/progress/.
...
Requires **CONTENT_MEMORY_ROOT** pointing at your corpus/topic folder; otherwise no files are written.
"""
...
def progress_dir(topic_root: Path, skill_name: str) -> Path:
    """Live checklists root: ``<CONTENT_MEMORY_ROOT>/<skill_name>/progress``."""
    return topic_root / skill_name / "progress"
...
def ensure_topic_checklists(skill: "Skill", phase_slug: str) -> tuple[Path | None, Path | None]:
    """
    If **CONTENT_MEMORY_ROOT** is set, ensure ``process-checklist.md`` and
    ``<phase_slug>-checklist.md`` exist under ``<topic_root>/<name>/progress/``.

    Returns ``(process_path_or_none, phase_path_or_none)`` for paths written this call.
    """
    wp = skill.instructions.context.topic_root
    if not wp:
        return None, None
    topic_root = Path(wp)
    ...
    prog = progress_dir(topic_root, skill_name)
    prog.mkdir(parents=True, exist_ok=True)

    written_process: Path | None = None
    written_phase: Path | None = None

    parts = _parts_dir(skill.path)
    phase_path = parts / "phases" / f"{phase_slug}.md"

    process_md = prog / "process-checklist.md"
    if not process_md.is_file() and phase_files:
        process_md.write_text(
            render_process_checklist(skill_name, phase_files, headings),
            encoding="utf-8",
        )
        written_process = process_md

    if phase_path.is_file():
        body = phase_path.read_text(encoding="utf-8")
        tasks = extract_action_checklist_lines(body)
        phase_check = prog / f"{phase_slug}-checklist.md"
        if not phase_check.is_file():
            phase_check.write_text(
                render_phase_checklist(skill_name, phase_slug, tasks),
                encoding="utf-8",
            )
            written_phase = phase_check

    return written_process, written_phase


# Backwards compatibility
ensure_workspace_checklists = ensure_topic_checklists
