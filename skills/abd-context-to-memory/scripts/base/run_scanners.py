"""
run_scanners.py — run the merged scanner set: optional **engine.scanners** (legacy: **workspace.scanners**) and
**build.scanners**, entries from **rules/scanners.json**, then every **scripts/scanners/*.py**.

Usage:
    python scripts/base/run_scanners.py [--skill-root <path>]
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to this skill package (directory containing skill-config.json)",
    )
    ...
    for scanner_path in scanners:
        ...
        result = subprocess.run(
            [sys.executable, str(script), "--skill-root", args.skill_root],
