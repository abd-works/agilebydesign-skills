...
# AGENTS preamble sections — fixed paths under ``content/parts/library/`` (or legacy ``parts/library/``).
...
def _resolve_library_md(parts_dir: Path, filename: str) -> Path | None:
    ...
    **Precedence:** ``library/<filename>`` (skill override) → ``library/required/<filename>``
    (scaffold templates / per-skill narrative) → ``library/base/<filename>`` (frozen norms from
    abd-skill-builder).
...
class Instructions:
    ...
    def _get_section_content(self, section_id: str, *, phase_slug: str | None = None) -> str:
        ...
        if section_id == f"{PREFIX}bundle.rules_empty":
            ...
            return (
                ...
                "`parts/library/process-phases.md` § Phase bundle — rules.*\n"
            )
    ...
    def _build_context_block(self) -> str:
        ...
        if self.context.topic_root:
            lines.append(
                f"**Topic root (CONTENT_MEMORY_ROOT):** `{self.context.topic_root}`"
            )
