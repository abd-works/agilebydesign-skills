...
- **Context:** Resolve from config. When `conf/abd-config.json` has `solution_workspace`, use `workspace/solution.conf` → `context_path` (default: `output_dir/context`). Read `context_chunks.json` from that directory. When no workspace, use `context/context_chunks.json` at skill root.
...
```
python scripts/mms_build_chunk_index.py
```
