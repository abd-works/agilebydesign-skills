...
Runs entirely inside the execute-skill-using-skills-rules skill — does not call scripts/base/build.py.
Idempotent: replaces content between HTML markers so re-runs do not duplicate text.

Usage (from anywhere):
  python skills/execute-skill-using-skills-rules/scripts/bundle_rules_into_skill_md.py --skill-root /path/to/skill
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Bundle rules/*.md into SKILL.md between execute_rules HTML comment markers."
    )
    p.add_argument(
        "--skill-root",
        type=Path,
    ...
    if not skill_md.is_file():
        print(f"[bundle-rules] Missing {skill_md}", file=sys.stderr)
        return 2
    ...
    print(f"[bundle-rules] Wrote bundled rules into {skill_md} ({len(entries)} rule file(s)).")
    return 0
