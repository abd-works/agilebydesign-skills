#!/usr/bin/env python3
"""Scanner: front matter must contain state: crc."""
...
class StateMarkerCorrectScanner(Scanner):
    """Check that module files at CRC stage carry state: crc."""
    ...
    def _scan_file(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        if not fm:
            return [Violation(
                rule=self.rule,
                violation_message="No YAML front matter found; expected state: crc",
        ...
        if not state_m:
            return [Violation(
                rule=self.rule,
                violation_message="No state: key in front matter; expected state: crc",
                location=str(file_path),
                line_number=1,
                severity="error",
            ).to_dict()]
