#!/usr/bin/env python3
"""Scanner: front matter must contain state: domain-sketch."""
...
_EXPECTED_STATE = "domain-sketch"


class StateMarkerCorrectScanner(Scanner):
    """Check that module files at domain-sketch stage carry state: domain-sketch."""
    ...
    def _scan_file(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        if not fm:
            return [Violation(
                rule=self.rule,
                violation_message="No YAML front matter found; expected state: domain-sketch",
        ...
        if not state_m:
            return [Violation(
                rule=self.rule,
                violation_message="No state: key in front matter; expected state: domain-sketch",
                location=str(file_path),
                line_number=1,
                severity="error",
            ).to_dict()]
        ...
        if actual != _EXPECTED_STATE:
            return [Violation(
                rule=self.rule,
                violation_message=f"state: '{actual}' — expected 'domain-sketch'",
                location=str(file_path),
                line_number=1,
                severity="error",
            ).to_dict()]
