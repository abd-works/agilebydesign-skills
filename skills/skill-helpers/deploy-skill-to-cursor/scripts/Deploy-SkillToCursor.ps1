...
.PARAMETER SkillName
  Folder name of the skill (e.g. story-graph-ops, abd-story-mapping).

.PARAMETER FamilyFolder
  Optional family subfolder under skills/ (e.g. abd-domain-driven-design).
  When set, resolves source as skills/<FamilyFolder>/<SkillName>.
  Ignored when SkillSourcePath is set.
...
.PARAMETER SkillSourcePath
  Full path to the skill folder when it is not under skills/<SkillName> or
  skills/<FamilyFolder>/<SkillName> (e.g. a completely custom path).
...
if ($SkillSourcePath) {
    ...
    $source = Join-Path $skillsDir (Join-Path $FamilyFolder $SkillName)
} else {
