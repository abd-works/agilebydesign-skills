"""
run_scanners.py — run all scanners listed in conf/abd-config.json and report results.

Usage:
    python scripts/base/run_scanners.py [--workspace <path>]

Reads conf/abd-config.json, iterates over the `scanners` list, runs each script,
and prints a summary. Exits 0 only if all scanners pass.

Scanners live in scripts/scanners/. Each scanner follows the contract:
    exit 0 = pass, exit non-zero = fail (details printed to stdout/stderr).
"""
...
def main() -> int:
    ...
    if not scanners:
        print("[INFO] No scanners configured in conf/abd-config.json")
        return 0
