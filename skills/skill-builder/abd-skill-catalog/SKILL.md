...
description: >-
  ...
  root `README.md` for card and detail copy (overview + ASCII), then regenerate
...
Maintain the browsable **AI Garden**: **skills** under the repo root
...
| **Hub / grid intros** | HTML fragments | `skills/skill-builder/abd-skill-catalog/templates/intros/*.html` (short lines above grids; optional polish) |
...
1. **Regenerate on command.** From the **agilebydesign-skills** repository root,
   ...
   ```bash
   python skills/skill-builder/abd-skill-catalog/scripts/generate_abd_catalog.py
   ```
   ...
   ```bash
   python skills/skill-builder/abd-skill-catalog/scripts/generate_abd_catalog.py --scaffold-readmes
   ```
...
5. **Hub and grid intros (review with an AI or editor).** Short HTML above the
   grids lives in `skills/skill-builder/abd-skill-catalog/templates/intros/`. Keep copy
...
7. **Templates (layout).** HTML shells and CSS live under
   `skills/skill-builder/abd-skill-catalog/templates/` (excluding `intros/`, which are prose
